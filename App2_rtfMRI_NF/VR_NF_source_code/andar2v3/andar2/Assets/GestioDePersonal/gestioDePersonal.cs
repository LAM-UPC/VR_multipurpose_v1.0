        ...
        if (principal)
        {
            ...
            if (infoProcesada)
            {
                for (int i = 0; i < info.Length / 12; i++)
                {
                    ...
                    GUI.Label(new Rect(8.75f * vy + refx, 5.75f * vy + refy + i * 2.5f * vy, 17 * vy, 0.75f * vy), "Puntuació: " + info[i, 5] + "  Asimetria: " + info[i, 6] + "%  Distancia: " + info[i, 7] + "m  Dret.Sup: " + info[i, 8] + "º  Izq.Sup: " + info[i, 9] + "º  Der.Inf: " + info[i, 10] + "º  Izq.Inf: " + info[i, 11] + "º");
